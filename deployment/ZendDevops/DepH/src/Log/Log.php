...
class Log extends \Zend\Log\Logger implements ListenerAggregateInterface
{
    /**
     ...
     * (non-PHPdoc)
     * 
     * Log evry deploymenet step
     * @see \Zend\EventManager\ListenerAggregateInterface::attach()
     */
    public function attach(EventManagerInterface $eventManager)
    {
        $eventManager->attach('*',array($this,'logEnterNewStep'),10);
    }
    ...
    /**
     * Log each time the process entering a new deployment step
     * @param \Zend\EventManager\Event $event
     */
    public function logEnterNewStep(\Zend\EventManager\Event $event)
    {
        $this->notice('Enter new step ' .$event->getName());
    }
}
